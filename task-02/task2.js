...
const students = [
  ...
  return students.map(student=>{
    const student={firstName,lastName,grades};
    const graevalue=Object.values(grades);
    const avg=graevalue.reduce((sum,grade)=>sum+grade,0);
    return {
        name:`${firstName}${lastName}`,
        averige:Math.round(avg*100)/100,
    }
  })
  }
  ...
  function getHighAchievers(students, gradeThreshold) {
    return students.map(student=>{
        ...
        const graevalue=Object.values(grades);
        const avg=graevalue.reduce((sum,grade)=>sum+grade,0);
        const students=student.filter(yes);
        return{
            name:`${firstName},${lastName}`,
        }
    })
  ...
  function getMultiAchivers(students, threshold) {
    return students.map(student=>{
        ...
        const student={firstName,lastName,grades};
        const graevalue=Object.values(grades);
        const cunter=function(){
            if(grade.Math>threshold){
                cnt++;
            };
            if(grade.history>threshold){
                cnt++;
            };
            if(grade.physics>threshold){
                cnt++;
            };
           return cnt; 
        }
        student=student.filter(cunter);
        return{
            name:`${firstName},${lastName}`,expertSubjects:`cnt`
        }
    })
  }
